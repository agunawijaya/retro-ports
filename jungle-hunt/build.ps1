...
#
#   1. comrec.py     original/hunt.com -> recovered/jungle-hunt.asm
...
param(
    ...
    [string]$Original = "original\hunt.com"
)
...
python (Join-Path $Toolkit "tools\comrec.py") $Original `
    --out recovered\jungle-hunt.asm --map recovered\jungle-hunt.map --nasm $Nasm
...
python (Join-Path $Toolkit "tools\annotate.py") `
    --asm recovered\jungle-hunt.asm --out recovered\jungle-hunt-named.asm --symbols symbols.json
if ($LASTEXITCODE -ne 0) { throw "annotate.py failed" }

Write-Host "3/3  rebuilding from the named source" -ForegroundColor Cyan
& $Nasm -f bin -o recovered\rebuilt.bin recovered\jungle-hunt-named.asm
