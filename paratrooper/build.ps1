...
#
#   1. comrec.py     original/ParaTrooper.1982.com -> recovered/paratrooper.asm
...
param(
    ...
    [string]$Original = "original\ParaTrooper.1982.com"
)
...
python (Join-Path $Toolkit "tools\comrec.py") $Original `
    --out recovered\paratrooper.asm --map recovered\paratrooper.map --nasm $Nasm
...
python (Join-Path $Toolkit "tools\annotate.py") `
    --asm recovered\paratrooper.asm --out recovered\paratrooper-named.asm --symbols symbols.json
if ($LASTEXITCODE -ne 0) { throw "annotate.py failed" }

Write-Host "3/3  rebuilding from the named source" -ForegroundColor Cyan
& $Nasm -f bin -o recovered\rebuilt.com recovered\paratrooper-named.asm
...
if ($a -eq $b) {
    ...
    Write-Host "recovered\paratrooper-named.asm rebuilds the game exactly."
} else {
