...
#
#   1. comrec.py     original/ZAXXON.COM -> recovered/zaxxon.asm
...
param(
    ...
    [string]$Original = "original\ZAXXON.COM"
)
...
python (Join-Path $Toolkit "tools\comrec.py") $Original `
    --out recovered\zaxxon.asm --map recovered\zaxxon.map --nasm $Nasm
...
python (Join-Path $Toolkit "tools\annotate.py") `
    --asm recovered\zaxxon.asm --out recovered\zaxxon-named.asm --symbols symbols.json
if ($LASTEXITCODE -ne 0) { throw "annotate.py failed" }

Write-Host "3/3  rebuilding from the named source" -ForegroundColor Cyan
& $Nasm -f bin -o recovered\rebuilt.com recovered\zaxxon-named.asm
...
if ($a -eq $b) {
    ...
    Write-Host "recovered\zaxxon-named.asm rebuilds the game exactly."
} else {
