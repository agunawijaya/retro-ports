...
export class Renderer {
    ...
    drawDailyMenu(gameState) {
        // The DOS game reuses whatever scene was last painted (the
        // arrival scene at a landmark, or the travel scene between)
        // as the backdrop, then overlays menu boxes. We mirror that:
        // always paint the current landmark's image as backdrop when
        // one is available; the menu boxes go on top of it.
        //
        // Between landmarks the "current" landmark is the last one
        // reached, which is exactly `currentLandmarkIndex`. Its image
        // is a reasonable stand-in for the travel scene -- and it's
        // what the DOS game shows in `05-daily-menu.png` at South Pass.
        const idx = Math.max(0, Math.min(gameState.currentLandmarkIndex, 17));
    ...
    /**
     * Position marker on the real map image. The (mapX, mapY) fields on
     * each landmark come from the leg records at DS:0x896 in the binary
     * (words at +0x21 and +0x23) -- so the marker lands where the DOS
     * game itself would draw it, projected into the letterboxed rect.
     */
    _drawMapMarker(gameState, rect) {
        const landmarks = gameState.landmarks;
        if (!landmarks) return;

        const MAP_W = 640, MAP_H = 400;   // vga_MAP.png dimensions

        // Draw the trail line from Independence up to the current
        // landmark ONLY -- future landmarks are on the map but not
        // yet connected. DOS uses a solid line for traveled portion
        // (proc_02D2B, `Graph:0x1670`), and no line ahead.
        const upto = Math.min(gameState.currentLandmarkIndex,
                              landmarks.length - 1);
        if (upto > 0) {
            ...
            for (let i = 0; i <= upto; i++) {
                const l = landmarks[i];
                const x = rect.x + (l.mapX / MAP_W) * rect.w;
                const y = rect.y + (l.mapY / MAP_H) * rect.h;
        ...
        // Marker at the current landmark. Interpolate toward the next
        // one by progress in miles inside the current leg.
        const here = landmarks[gameState.currentLandmarkIndex];
        const nextIdx = (gameState.nextIndexOverride != null)
            ? gameState.nextIndexOverride : here.next;
        const next = landmarks[nextIdx];
        let mx = here.mapX, my = here.mapY;
        if (next) {
            const legMiles = Math.max(1, next.miles - here.miles);
            const inLeg = Math.max(0, Math.min(1,
                (gameState.totalMiles - here.miles) / legMiles));
            mx = here.mapX + (next.mapX - here.mapX) * inLeg;
            my = here.mapY + (next.mapY - here.mapY) * inLeg;
        }
        const x = Math.floor(rect.x + (mx / MAP_W) * rect.w);
        const y = Math.floor(rect.y + (my / MAP_H) * rect.h);
    ...
    /**
     * The keypad-driven hunting field from hunting.js. `field` is
     * `{ hunter, scenery, animals }` in canvas coordinates. Scenery
     * and animals are drawn as coloured boxes -- ANIMALS spritesheet
     * has the right sprites but the per-species column layout would
     * need porting the sprite table; boxes are honest about being a
     * placeholder.
     */
    drawHuntingField(field, result) {
        // Sky + ground band. Rough approximation of the BGI 320x200
        // hunting field (docs/03: "the field is a BGI 320x200 screen").
        this.ctx.fillStyle = '#87ceeb';
        this.ctx.fillRect(0, 0, this.width, 120);
        this.ctx.fillStyle = '#5c8f3e';
        this.ctx.fillRect(0, 120, this.width, this.height - 120);

        // Scenery -- blit from vga_TERRAIN.png using the sprite table
        // read out of DS:0x013A. Each scenery object carries its own
        // (sx, sy, w, h) from TERRAIN_KINDS.
        for (const s of field.scenery) {
            if (s.sx != null) {
                this.assets.drawSprite(
                    this.ctx, ASSET_KEYS.TERRAIN,
                    { sx: s.sx, sy: s.sy, sw: s.w, sh: s.h },
                    s.x, s.y, s.w, s.h,
                );
            } else {
        ...
        // Animals -- blit the measured frame from vga_ANIMALS.png.
        // ANIMALS in assets.js has per-species frames[0..5] whose
        // sx/sy/sw/sh were programmatically derived from the PNG.
        const now = performance.now();
        for (const a of field.animals) {
            if (!a.alive) continue;
            const sp = a.species;
            if (sp && sp.frames && sp.frames.length) {
                const idx = Math.floor(now / (sp.animFrameInterval || 120))
                          % sp.frames.length;
                const frame = sp.frames[idx];
                this.assets.drawSprite(
                    this.ctx, ASSET_KEYS.ANIMALS, frame,
        ...
        // Hunter -- HUNTER_SPRITES.getSprite picks a (row, col) from
        // the aim direction. It was written for mouse dx/dy so we
        // synthesise a mouse-like offset from the keypad direction.
        const h = field.hunter;
        const cx = h.x + h.w / 2;
        const cy = h.y + h.h / 2;
        const mx = cx + h.direction.dx * 20;
        const my = cy + h.direction.dy * 20;
        const sprite = HUNTER_SPRITES.getSprite(mx, my, cx, cy);
        this.assets.drawSprite(
            this.ctx, HUNTER_SPRITES.sourceKey, sprite,
            Math.floor(h.x), Math.floor(h.y),
            sprite.sw, sprite.sh,
        );
        ...
        this.ctx.font = '10px monospace';
        this.ctx.fillText(
            `Hits ${result.hits}  Shots ${result.shotsFired}  Meat ${result.meat}lb`,
            4, 10,
        );
        this.ctx.fillText(
            'Enter=walk  Space=fire  Esc=stop  1-9=aim',
            4, this.height - 3,
        );
    }
     ...
     */
    /**
     * River-crossing scene composited the way proc_055BA does it:
     * clear to a background, paint a water rectangle in blue at the
     * DOS coordinates (80, 25) to (240, 150), then blit one of three
     * wagon-on-water sprites from vga_FLOAT.png based on `outcome`
     * (0 = safe, 1 = tipped nothing lost, 2 = tipped supplies lost).
     * With no outcome yet (still choosing), we draw the "safe"
     * sprite as the neutral pose.
     *
     * @param {{name:string, depth?:number, widthFt?:number, outcome?:number}} info
     */
    drawRiverCrossing(info) {
        // Grass-brown background outside the water strip.
        this.clearScreen('#5c8f3e');

        // Water rectangle at DOS coordinates (proc_055BA image 0x055F2).
        // Colour chosen to match the FLOAT sprites' own background pixel
        // rgb(64, 176, 252) so the wagon blends seamlessly onto the
        // water strip -- otherwise the sprite's own light-blue bleeds
        // through as a visible rectangle.
        const wr = FLOAT_SPRITES.waterRect;
        this.ctx.fillStyle = '#40b0fc';
        this.ctx.fillRect(wr.x, wr.y, wr.w, wr.h);

        // Wagon sprite for the current outcome, anchored so the bottom
        // sits at anchorY (110 in DOS, image 0x056DD `mov ax, 0x6e`).
        const outcomeIdx = Math.max(0, Math.min(2, info.outcome ?? 0));
        const sprite = FLOAT_SPRITES.outcomes[outcomeIdx];
        const dx = wr.x + Math.floor((wr.w - sprite.sw) / 2);
        const dy = FLOAT_SPRITES.anchorY - sprite.sh;
        this.assets.drawSprite(
            this.ctx, FLOAT_SPRITES.sourceKey, sprite,
            dx, dy, sprite.sw, sprite.sh,
        );

        // Info panel top-left with river name / width / depth.
        if (info && info.name) {
            const lines = [info.name];
            if (info.widthFt != null) lines.push(`${info.widthFt} ft wide`);
            if (info.depth   != null) lines.push(`${info.depth} ft deep`);
