...
#
#   1. comrec.py     original/CAT.EXE -> recovered/alley-cat.asm
...
param(
    ...
    [string]$Original = "original\CAT.EXE"
)
...
python (Join-Path $Toolkit "tools\annotate.py") `
    --asm recovered\alley-cat.asm --out recovered\alley-cat-named.asm --symbols symbols.json
if ($LASTEXITCODE -ne 0) { throw "annotate.py failed" }

Write-Host "3/3  rebuilding from the named source" -ForegroundColor Cyan
& $Nasm -f bin -o recovered\rebuilt.bin recovered\alley-cat-named.asm
...
$hdr = Join-Path $here "recovered\alley-cat.mzheader"
