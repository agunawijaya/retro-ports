...
#
#   1. comrec.py     original/FROGGER.COM -> recovered/frogger.asm
...
param(
    ...
    [string]$Original = "original\FROGGER.COM"
)
...
python (Join-Path $Toolkit "tools\comrec.py") $Original `
    --out recovered\frogger.asm --map recovered\frogger.map --nasm $Nasm
...
python (Join-Path $Toolkit "tools\annotate.py") `
    --asm recovered\frogger.asm --out recovered\frogger-named.asm --symbols symbols.json
if ($LASTEXITCODE -ne 0) { throw "annotate.py failed" }

Write-Host "3/3  rebuilding from the named source" -ForegroundColor Cyan
& $Nasm -f bin -o recovered\rebuilt.com recovered\frogger-named.asm
...
if ($a -eq $b) {
    ...
    Write-Host "recovered\frogger-named.asm rebuilds the game exactly."
} else {
