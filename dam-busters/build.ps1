...
#
#   1. comrec.py     original/DAMB.EXE -> recovered/dam-busters.asm
...
param(
    ...
    [string]$Original = "original\DAMB.EXE"
)
...
python (Join-Path $Toolkit "tools\comrec.py") $Original `
    --out recovered\dam-busters.asm --map recovered\dam-busters.map --nasm $Nasm
...
python (Join-Path $Toolkit "tools\annotate.py") `
    --asm recovered\dam-busters.asm --out recovered\dam-busters-named.asm --symbols symbols.json
if ($LASTEXITCODE -ne 0) { throw "annotate.py failed" }

Write-Host "3/3  rebuilding from the named source" -ForegroundColor Cyan
& $Nasm -f bin -o recovered\rebuilt.bin recovered\dam-busters-named.asm
...
$hdr = Join-Path $here "recovered\dam-busters.mzheader"
