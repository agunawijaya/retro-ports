...
**Reading well underway, still byte-identical.** `symbols.json` holds **168
routines and 127 globals**, each with the evidence for its name.
`annotate.py` reports:
...
BYTE-IDENTICAL. wrote recovered\dam-busters.asm
              ...
              ...
168 routine names, 127 globals
  applied: ...
  names 104 of 158 call targets
  no unnamed tail-call entries
  covers 111 of 433 bracketed constants
  no _data_spans: the bytes between the named addresses are not accounted for
