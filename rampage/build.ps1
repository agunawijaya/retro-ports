...
#
#   1. comrec.py     original/Rampage.exe -> recovered/rampage.asm
...
param(
    ...
    [string]$Original = "original\Rampage.exe"
)
...
python (Join-Path $Toolkit "tools\comrec.py") $Original `
    --out recovered\rampage.asm --map recovered\rampage.map --nasm $Nasm
...
python (Join-Path $Toolkit "tools\annotate.py") `
    --asm recovered\rampage.asm --out recovered\rampage-named.asm --symbols symbols.json
if ($LASTEXITCODE -ne 0) { throw "annotate.py failed" }

Write-Host "3/3  rebuilding from the named source" -ForegroundColor Cyan
& $Nasm -f bin -o recovered\rebuilt.bin recovered\rampage-named.asm
...
$hdr = Join-Path $here "recovered\rampage.mzheader"
